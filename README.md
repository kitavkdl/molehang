...
무한의 바다 위에 배 한 척. 시간이 지나면 고철이 알아서 쌓이고, 수거한 고철로
뽑기를 돌리면 **무작위 부품이 나와서 전부 그대로 배에 붙는다.** 고를 수 없다.
...
**코어 루프**
- 생산량은 배에 달린 부품에서 나온다(빈 배는 초당 0.5). 상한 = 초당 생산량 × 30분.
  창을 닫아도 흐르는 오프라인 축적
- 수거 → 물 위의 고철 조각이 갑판 상자로 빨려 들어가는 파티클 + 배 바운스
- 타임스탬프 로그와 기록 시트

**파츠 & 시크릿 전직**
- 부품 31종 — 3등급(작음·중간·대형) 돌림판에서 가중치 확률로 나온다.
  낮은 확률의 밸런스 붕괴 부품(황금 오리 …)과 항해·방치로만 붙는 3종(따개비·둥지·유령)도 있다
- **나온 건 전부 장착된다.** 자리가 차면 위로 쌓인다 — 엔진 12개면 4층 엔진탑.
  희소 자원은 고철이 아니라 **선체 자리**다 — 자리가 모자라면 그 자리에서 뺄 부품을 고른다
- 구성에 따라 숨은 칭호 15종 (이끼 유령선, 폭주 기관선, 돛의 탑, 강철 요새 …)

**망원경 & 항해모드**
- 배율 0.4~4 망원경 — 줄이면 망망대해, 키우면 부품 하나. 확대 상태에서 끌어서 이동
- 항해모드 — WASD·화면 끌기로 몰고, 뱃머리가 다 돌아야 최고 속도(체이스 캠).
  암초에 부딪히면 따개비가 붙고 선체가 상한다(정박하면 아문다). 침몰·게임오버는 없다
- 배치 커스텀 — 구역 안에서 부품을 끌어 옮긴다. 닿아 있어야 붙고, 균형은 안 따진다
- 방치 컨텐츠 — 오래 비우면 이끼·갈매기 둥지·유령 선원이 저절로 붙는다
- 선장 아바타 — 모자·옷 색을 고르면 갑판 위에 선다. 선단 접속 중엔 선원들도 나란히

**선단 — 혼자도 되지만 같이 하면 빨라진다**
- 초대 코드(6자리)로 최대 4명까지 묶인다. 링크 하나로 초대
- **같이 접속해 있는 동안** 축적 속도 1인당 +15% (최대 +45%)
- 친구가 수거하면 그 양의 12%가 **고철 배당**으로 내 지갑에 들어온다
- **동행선** — 선원들의 배가 내 바다에 떠서 나란히 항해한다 (돛 줄무늬 = 그 사람 옷 색)
- **만선 콤보** — 60초 안에 서로 수거하면 양쪽 다 자기 수거량의 +30%
- **순풍** — 같이 있는 동안 뽑기가 12%/명(최대 36%) 확률로 한 등급 위 돌림판로 승급
...
src/
  ...
  game/local-gateway.ts  sessionStorage 구현 (게스트 — 탭을 닫으면 사라진다)
...
UI·씬 코드는 저장소를 직접 만지지 않으므로 호출부는 바뀌지 않는다.
